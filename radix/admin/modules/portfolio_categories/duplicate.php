...
if (!empty($body['id'])) {
    $cateId = $body['id'];
    $query = firstRaw("SELECT * FROM portfolio_categories WHERE id = $cateId");
    if (!empty($query)) {
        ...
        $status = insert('portfolio_categories', $query);
        if ($status) {
            setFlashData('msg', 'Nhân bản danh mục thành công');
            setFlashData('msg_type', 'success');
            update('portfolio_categories', ['duplicate' => $duplicate], "id=$cateId");
        } else {
